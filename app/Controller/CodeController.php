...
class CodeController extends BaseController
{
    ...
    public function getcodelistdata(RequestInterface $request, ResponseInterface $response)
    {
        ...
        $data = CodeDetailUpdate::getCodeList($spn, $keyword, $page, $page_size, $one_class, $two_class);
    ...
    public function showguide(RequestInterface $request, RenderInterface $render)
    {
        ...
        if (empty($id) || empty($text)) {
            ...
            if (empty($data)) {
                MyLog::save_data($id, $text);
                return  $this->error(1004, '真的没有相关内容');

            }else if(empty($data['content'])){
                return  $this->error(1004, '没有相关内容');
            }else{
                return $this->success($data['content'],'登陆成功！');
            }
        }
